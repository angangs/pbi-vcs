# 1: have scripts which extract from .pbit to .pbit.extract - gitignore .pbit (and .pbix), AND creates .pbix.chksum
# (which is only useful for versioning purposes - one can confirm the state of their pbix)
# - script basically extracts .pbit to new folder .pbit.extract, but a) also extracts double-zipped content, and b)
# formats stuff nicely so it's readable/diffable/mergeable.
# 2: have git hooks which check, before a commit:
# - checks that the .pbit.extract folder is up to date with the latest .pbit (i.e. they match exactly - and the
# .pbit hasn't been exported but user forgot to run the extract script)
# - adds a warning (with y/n continue feedback) if the .pbix has been updated *after* the latest .pbit.extract is
# updated. (I.e. they maybe forgot to export the latest .pbit and extract, or exported .pbit but forgot to extract.)
# Note that this will be obvious in the case of only a single change (as it were) - since .pbix aren't tracked,
# they'll see no changes to git tracked files.
...
os.chdir(os.getcwd() + './powerbi-vcs/pbi-vcs')
...
input_file = './samples/SM_AP_Landing_Page.pbit'
output_file = './samples/SM_AP_Landing_Page_out.pbit'
extract_pbit(input_file, output_file, 'True')

# textconv_pbit(input_file, sys.stdout)

input_file = './samples/SM_AP_Landing_Page_out.pbit'
output_file = './samples/SM_AP_Landing_Page_out_2.pbit'
